...
Node *copyList(Node *h) {
    Node *head=h;
    Node *front;
    // Inserting deep copy nodes as next
    while(head!=NULL)
    {
       front=head->next;
     ...
     // Inserting random pointers in deep copy nodes
    head=h;
    while(head!=NULL)
    {
        if(head->arb!=NULL)
    ...
    // Now separating both the deep copy list and original list
    head=h;
    //make a dummy node and randomly initialise it
